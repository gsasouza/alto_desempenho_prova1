...
void pretty_print_output(
  ...
  for (int i = 0; i < size_regions; i++) {
    ...
    for (int j = 0; j < size_cities; j++) {
      ...
      printf("Reg %d - Cid %d: menor: %d, maior: %d, mediana: %.2f, média: %.2lf e DP: %.2lf\n", i, j,
             ...
             city_highest[position], rounded(city_median[position]), rounded(city_mean[position]), rounded(city_standard_deviation[position]));
    }
  ...
  for(int i = 0; i < size_regions; i++) {
    printf("Reg %d: menor: %d, maior: %d, mediana: %.2f, média: %.2lf e DP: %.2lf\n", i, region_lowest[i], region_highest[i], rounded(region_median[i]), rounded(region_mean[i]), rounded(region_standard_deviation[i]));
  }
